...
public class Login implements WindowListener, ActionListener
{
	...
	Dialog dlgMensaje = new Dialog(ventanaLogin, "Error", true); //true: indica que es modal, no podemos interactuar con nada hasta que no cerremos ese diálogo
	...
	{
		if(e.getSource().equals(btnAcceder)) { //si hay dos botones, utilizamos esta instrucción
	...
	public void windowClosing(WindowEvent e)
	{
		if(dlgMensaje.isActive()) { 
			dlgMensaje.setVisible(false); //para que no se cierre el login cuando cierres el dialog
		}
		else {
			System.exit(0);
		}
